...
void update_CommandBlock(struct CommandBlock &command_block,
  ...
  command_block.ux = gain_K * (command_block.x_ref - command_block.x_head) +
                     command_block.dx_ref;
  command_block.uy = gain_K * (command_block.y_ref - command_block.y_head) +
                     command_block.dy_ref;
}

void update_SaturationBlock(struct SaturationBlock &bloque,
                            struct RobotParamater &param) {
...
void update_integrator(struct IntegratorBlock &itegrator_block,
                       struct TurtlebotBlock &turtlebot2, double dt) {
  ...
  itegrator_block.theta =
      (itegrator_block.dtheta + last_dtheta) * (dt / 2.0f) + somme_theta;
...
void update_TurtlebotBlock(struct TurtlebotBlock &turtlebot2,
...
void update_M_inverseBlock(struct M_inverseBlock &inverse_matrix_block,
                           struct CommandBlock &command_block,
                           struct RobotParamater &param, double theta) {
  ...
  double M_inv[2][2] = {
      ...
      {(-1 / param.R) * sin(inverse_matrix_block.theta),
       (1 / param.R) * cos(inverse_matrix_block.theta)}};

  inverse_matrix_block.v_desired = M_inv[0][0] * inverse_matrix_block.ux +
                                   M_inv[0][1] * inverse_matrix_block.uy;
  inverse_matrix_block.w_desired = M_inv[1][0] * inverse_matrix_block.ux +
                                   M_inv[1][1] * inverse_matrix_block.uy;
}

void update_GainSpeedBlock(struct GainSpeedBlock &gain_speed_block,
  ...
  gain_speed_block.v_turtle =
      gain_speed_block.v_desired * gain_speed_block.pre_gain_v;
  gain_speed_block.w_turtle =
      gain_speed_block.w_desired * gain_speed_block.pre_gain_w;
}

void update_TrajectoryOutput(struct GenerationTrajectoireBlock &bloque,
                             double time_current) {
  ...
  a[3] =
      ((bloque.vf - bloque.v0) * (bloque.tf - bloque.t0)) -
      (2 * (bloque.pf - bloque.p0) - (2 * bloque.v0 * (bloque.tf - bloque.t0)));
  ...
  if (time_current < bloque.tf) {
    ...
    bloque.dq = (a[1] + 2 * a[2] * lambda + 3 * a[3] * lambda_2) /
                (bloque.tf - bloque.t0);
  } else {
    bloque.q = bloque.pf;
    bloque.dq = bloque.vf;
...
void update_CoordsHead(struct ReferenceChangeBlock &reference_change_block,
  ...
  // Update output
  reference_change_block.x_head =
      reference_change_block.x + param.R * cos(reference_change_block.theta);
  reference_change_block.y_head =
      reference_change_block.y + param.R * sin(reference_change_block.theta);
